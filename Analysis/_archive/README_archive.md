...
`13_nonlinear_systems_dynamics.R` and `14_nextgen_behavioral_phenotyping.R`
were relocated to `Analysis/_supporting/`. They are not executed by
`run_all_analysis.R`, but the active Stage 10 and Stage 14 scripts READ the
output trees they produce, so describing them as archived was misleading. See
`Analysis/_supporting/README.md` and `docs/ARCHIVE_DEPENDENCY_AUDIT.csv`.
