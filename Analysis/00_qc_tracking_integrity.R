...
read_file_header <- function(path) {
  ext <- tolower(tools::file_ext(path))
  header <- tryCatch(
    switch(
      ext,
      "csv" = names(readr::read_csv(path, n_max = 0, show_col_types = FALSE)),
      "tsv" = names(readr::read_tsv(path, n_max = 0, show_col_types = FALSE)),
      "txt" = names(readr::read_delim(path, delim = "\t", n_max = 0, show_col_types = FALSE)),
      "rds" = names(readRDS(path)),
      character()
    ),
    error = function(e) character()
  )
  names(standardize_names(as.data.frame(setNames(as.list(rep(NA, length(header))), header))))
}

has_required_qc_columns <- function(path) {
  header <- read_file_header(path)
  has_animal <- any(c("AnimalID", "AnimalNum") %in% header)
  has_movement <- "Movement" %in% header
  has_animal && has_movement
}
...
find_candidate_files <- function(search_dirs = SEARCH_DIRS) {
  ...
  compatible <- purrr::keep(candidates, has_required_qc_columns)
  skipped <- setdiff(candidates, compatible)
  if (length(skipped) > 0) {
    skipped_tbl <- tibble::tibble(
      path = skipped,
      reason = "missing AnimalID/AnimalNum or Movement after standardizing column names"
    )
    safe_dir_create(file.path(OUT_DIR, "manifest"))
    readr::write_csv(skipped_tbl, file.path(OUT_DIR, "manifest", "skipped_input_candidates.csv"))
    message("Skipped incompatible QC candidates: ", length(skipped),
            " (see ", file.path(OUT_DIR, "manifest", "skipped_input_candidates.csv"), ")")
  }
  compatible
}
...
if (exists("analysis_output_dirs")) {
  output_dirs <- analysis_output_dirs(OUT_DIR)
  safe_dir_create(output_dirs$tables)
  safe_dir_create(output_dirs$figure_qc)
} else {
  safe_dir_create(file.path(OUT_DIR, "figures", "qc"))
  safe_dir_create(file.path(OUT_DIR, "tables"))
  safe_dir_create(file.path(OUT_DIR, "manifest"))
  output_dirs <- list(tables = file.path(OUT_DIR, "tables"), figure_qc = file.path(OUT_DIR, "figures", "qc"))
}
...
# If INPUT_FILES contains directories, expand them to files inside.
INPUT_FILES <- unlist(lapply(INPUT_FILES, function(p) {
  if (dir.exists(p)) {
    list.files(p, pattern = "\\.(csv|tsv|txt|rds)$", recursive = TRUE, full.names = TRUE, ignore.case = TRUE)
  } else {
    p
  }
}))
...
figures_dir <- file.path(OUT_DIR, "figures", "qc")
...
if (exists("write_output_manifest")) {
  write_output_manifest(
    OUT_DIR,
    script_name = "00_qc_tracking_integrity.R",
    ...
    primary_tables = c(
      ...
      "tables/tracking_qc_rfid_loss_report.xlsx"
    ),
    primary_figures = c(
      "figures/qc/qc_zero_movement_fraction_heatmap.svg",
